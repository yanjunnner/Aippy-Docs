...
description: "Learn how to integrate Replicate with your Aippy application"
icon: "robot"
---
...
## Why use Replicate with Aippy?

Replicate fits naturally into Aippy’s AI-first app-building workflow. You can:
...
With Aippy’s built-in knowledge of Replicate’s API and models, integration takes minutes—not days. Just describe what you want, and Aippy handles the rest.

## Step by Step Tutorial

In this tutorial, we walk through how to integrate Replicate into a Aippy application to dynamically generate course banner images, adding a new layer of interactivity and polish to your product. You’ll also learn how Replicate fits into Aippy’s broader AI workflow—including how to pair it with OpenAI for course content, Superbase for backend logic, and real-time AI conversations using [OpenAI’s WebRTC API](https://platform.openai.com/docs/guides/realtime#connect-with-webrtc).
...
<Steps>
  <Step title="Step 1 – Build a Language Tutor App">
    We start by creating a Spanish-learning web app with Aippy. This includes:
  ...
  <Step title="Step 3 – Add Visuals with Replicate">
    ...
    **Example Response Handling:**
    ...
    Some Replicate models return a single image URL, others return an array. Make sure your Aippy function correctly extracts the output, e.g.:
  ...
  <Step title="Step 4 – Use Replicate Playground for Fine-Tuning">
    ...
    - Copy-paste directly into Aippy's backend functions
  ...
  <Step title="Step 5 – Real-Time Conversations with OpenAI">
    ...
    This feature integrates smoothly with Aippy’s voice input, WebRTC handling, and frontend chat UI.
...
- **Backend Logs**: Use Supabase Edge logs to debug your API calls. Aippy supports in-app log fetching.
- **Version Control in Aippy**: Each prompt edit is auto-committed, but you can manually track checkpoints using the “Deploy” feature for production-ready states.
...
<AccordionGroup>
  ...
  <Accordion title="Do I need my own Replicate API key?">
    Yes. Aippy securely stores your keys and handles API calls for you once added.
  </Accordion>
  ...
  <Accordion title="What’s the difference between Replicate's old and new API endpoints?">
    ...
    Aippy integrates with both depending on the model’s requirements.
  </Accordion>
  ...
  <Accordion title="Does Aippy know how to work with Replicate out of the box?">
    Yes\! Aippy has built-in knowledge about Replicate and many of its popular models. It can auto-generate integration code for you using the right packages and prompt structure.
  </Accordion>
  <Accordion title="How does Aippy handle package installation like replicate for Node.js?">
    Aippy automatically installs and configures packages when needed, based on your prompt and integration context. You don’t have to npm install manually unless debugging locally.
  </Accordion>
  <Accordion title="What’s the workflow for debugging Replicate errors in Aippy?">
    - Use Aippy’s **Superbase Edge function logs** to trace issues.
    - If there’s a mismatch between expected vs. actual Replicate response, update your JSON handling.
    - Use the **“Fix this”** button in Aippy to retry or refactor the function logic.
  </Accordion>
  <Accordion title="Can I checkpoint my working app state in Aippy?">
  ...
  <Accordion title="How does GitHub integration work with Aippy?">
    You can export your Aippy project to GitHub with **two-way sync**:

    - Aippy pushes changes to GitHub
    - You (or your team) can make changes in an IDE and push back
    - Works great for frontend in Aippy \+ backend in your own editor
  </Accordion>
  <Accordion title="What are LLM-friendly .lm.txt or .lm.md files?">
    These are simplified markdown-based docs optimized for language models. Replicate is actively working on adding these to help Aippy (and other tools) better understand its models automatically.
  </Accordion>
  <Accordion title="Do I need to handle prompt tuning myself?">
    Yes, prompt crafting is essential. Aippy helps with auto-prompting and templating, but you should always test in the Replicate Playground to refine the inputs for best results.
  </Accordion>
</AccordionGroup>
